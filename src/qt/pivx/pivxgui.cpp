...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Blockade client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Blockade"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Blockade"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
