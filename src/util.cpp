...
const char * const PIVX_CONF_FILENAME = "blockade.conf";
const char * const PIVX_PID_FILENAME = "blockade.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Blockade only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\blockade
// Windows >= Vista: C:\Users\Username\AppData\Roaming\blockade
// Mac: ~/Library/Application Support/blockade
// Unix: ~/.blockade
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blockade";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\blockadeParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\blockadeParams
    // Mac: ~/Library/Application Support/blockadeParams
    // Unix: ~/.blockade-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlockadeParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "BlockadeParams";
#else
    // Unix
    return pathRet / ".blockade-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty blockade.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override blockade.conf
