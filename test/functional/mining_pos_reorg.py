...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected ADEC and zADEC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zADECsupply']:
                assert_equal(node['zADECsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check ADEC and zADEC supply at the beginning
        # ------------------------------------------
        # zADEC supply: 2 coins for each denomination
        ...
        # ADEC supply: block rewards minus burned fees for minting
        ...
        # Verify that ADEC and zADEC supplies were properly updated after the spends and reorgs
        self.log.info("Check ADEC and zADEC supply...")
